...
/**
 * A dialog for adding or editing a new password record.
 */
public abstract class AddEditPasswordRecordDialog extends AddEditDialog<IPasswordRecord> {
     ...
     * @param factory The portunus-factory to use  for adding/editing the record
     */
    ...
    protected void assembleContentPane(GridPane gridPane) {
        ...
        gridPane.add(notesLabel, 0, 6);
        GridPane.setValignment(notesLabel, VPos.TOP);
        gridPane.add(notesTextArea, 1, 6, 2, 1);
    ...
    private int getSecondsToNextSecondDivisor(int divisor) {
        long secondsUtc = System.currentTimeMillis() / 1000;
        return (int) (divisor - (secondsUtc % divisor));
    }
